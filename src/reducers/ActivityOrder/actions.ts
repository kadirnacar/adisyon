...
export const actionCreators = {
  sendItem: (data: IActivityOrder, ecrid) => async (dispatch, getState) => {
    let isSuccess: boolean = false;
    await batch(async () => {
      await dispatch({type: Actions.RequestSendActivityOrderItems});
      var result = await ActivityOrderService.sendItem(data, ecrid);
  ...
  checkItem: (activity: IActivity, seance: ISeance, guestId: string) => async (
    dispatch,
    getState,
  ) => {
    let resultSet: any = {};
    await batch(async () => {
      ...
      var result = await ActivityOrderService.checkkItem(
        activity,
        seance,
        guestId,
      );
      ...
      if (isRequestSuccess) resultSet = result.value[0][0];
    });
  ...
  setCheckItem: (activity: IActivity, seance: ISeance) => async (
    dispatch,
    getState,
  ) => {
    let isSuccess: boolean = false;
